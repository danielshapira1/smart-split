...
import { Plus, LogOut, Users, Link2, Trash2, Pencil, ChevronDown, Wallet } from 'lucide-react'
  ...
  return (
    <div className='max-w-md mx-auto h-full flex flex-col'>
      <header className='sticky top-0 z-10 bg-white border-b px-4 py-3 flex items-center gap-2'>
        <GroupSwitcher groups={groups} current={group} onSelect={setGroup} onCreated={g => { setGroups(prev => [g, ...prev]); setGroup(g); }} />
      ...
      <div className='px-4 pt-3 flex gap-2'>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder='חיפוש תיאור...' className='flex-1 rounded-xl border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black'/>
        <select value={category} onChange={e=>setCategory(e.target.value)} className='rounded-xl border px-3 py-2 text-sm outline-none'>
      ...
      <nav className='px-4 pt-2 flex gap-2'>
        <button onClick={()=>setTab('expenses')} className={clsx('px-3 py-2 rounded-full text-sm', tab==='expenses'?'bg-black text-white':'bg-slate-100')}>הוצאות</button>
        <button onClick={()=>setTab('balances')} className={clsx('px-3 py-2 rounded-full text-sm', tab==='balances'?'bg-black text-white':'bg-slate-100')}>מאזנים</button>
      </nav>
              ...
              {filtered.map(e => (
                <li key={e.id} className='bg-white rounded-2xl shadow p-3 flex items-center justify-between'>
                  <div>
                    ...
                    <div className='text-xs text-gray-500'>קטגוריה: {e.category} · {new Date(e.occurred_on).toLocaleDateString('he-IL')} · שולם ע"י {e.payer_name || e.user_id}</div>
                  </div>
      ...
      {tab==='expenses' && (
        <footer className='sticky bottom-0 bg-white border-t p-3'>
          <div className='flex items-center justify-between'>
            <div className='text-gray-600 text-sm'>סה"כ: <span className='font-semibold'>₪{(total/100).toFixed(2)}</span></div>
            <button
              onClick={() => setShowForm(true)}
              className={clsx('px-4 py-2 rounded-full shadow bg-black text-white flex items-center gap-1 active:scale-[.98]')}
            >
  ...
  return (
    <div className='h-full max-w-sm mx-auto flex flex-col items-center justify-center p-4'>
      ...
      <div className='w-full space-y-3'>
        <button onClick={google} className='w-full rounded-xl bg-red-600 text-white py-3 font-medium active:scale-[.99]'>התחברות עם Google</button>
        ...
        <form onSubmit={sendMagic} className='w-full space-y-3'>
          ...
          />
          <button className='w-full rounded-xl bg-black text-white py-3 font-medium active:scale-[.99]'>שלח לי קישור</button>
        </form>
      </div>
